...
class One_Time_Purchases extends BaseController
{
    public function index()
    {
        ...
        if ($this->request->getMethod() == 'post') {
            if ($this->request->getPost('delete') === "true") {
                ...
                $db->transComplete();
                return redirect()->to('one_time_purchases');
            }
            ...
            if (!$this->validate($rules)) {
                ...
                if ($this->request->getPost('update') == "-1") {
                    $db->query("INSERT INTO purchases(recurring, purchase, amount, user_id) VALUES (false, \"" . $this->request->getPost('purchase') . "\", " . $this->request->getPost('amount') . ", " . session()->get('id') . ")");
                } else {
        ...
        $data['purchases'] = $db->query("SELECT * from purchases WHERE user_id=" . session()->get('id') . " AND recurring=false");

        echo view('templates/header', $data);
        echo view('one_time_purchases');
