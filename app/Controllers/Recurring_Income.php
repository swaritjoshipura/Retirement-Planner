...
class Recurring_Income extends BaseController
{
    public function index()
    {
        ...
        if ($this->request->getMethod() == 'post') {
            if ($this->request->getPost('delete') === "true") {
                ...
                $db->transComplete();
                return redirect()->to('recurring_income');
            }
            ...
            if (!$this->validate($rules)) {
                ...
                if ($this->request->getPost('update') == "-1") {
                    $db->query("INSERT INTO income(recurring, source, amount, user_id) VALUES (true, \"" . $this->request->getPost('source') . "\", " . $this->request->getPost('amount') . ", " . session()->get('id') . ")");
                } else {
        ...
        $data['income'] = $db->query("SELECT * from income WHERE user_id=" . session()->get('id') . " AND recurring=true");

        echo view('templates/header', $data);
        echo view('recurring_income');
